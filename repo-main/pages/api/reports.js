...
function sanitizePaypalLink(link) { if (!link || typeof link !== "string") throw new Error("Invalid PayPal link"); let l = link.trim().replace(/^https?:\/\//i, "").replace(/^www\./i, ""); const regex = /^paypal\.me\/[A-Za-z0-9._-]+(\/?[0-9.,]*)?$/; if (!regex.test(l) || /[<>{}()]/.test(l)) throw new Error("Invalid PayPal link format"); return "https://" + l.replace(/^\/+/, ""); }
